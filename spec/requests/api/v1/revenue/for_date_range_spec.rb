...
RSpec.describe 'requests for date range' do
  before :each do
    ...
    @invoice_1 = create(:invoice, customer: @customer, merchant: @merchant, status: 'shipped', updated_at: Time.parse('2021-06-01')) # should be counted and in date range
    ...
    @invoice_2 = create(:invoice, customer: @customer, merchant: @merchant, status: 'pending', updated_at: Time.parse('2021-05-01')) # not shipped
    @transaction_2 = create(:transaction, invoice: @invoice_2, result: 'success') 
    @invoice_item_3 = create(:invoice_item, invoice: @invoice_2, item: @item_3, unit_price: 20.00)

    @invoice_3 = create(:invoice, customer: @customer, merchant: @merchant, status: 'shipped', updated_at: Time.parse('2021-05-01')) # bad transaction
    ...
    @invoice_4 = create(:invoice, customer: @customer, merchant: @other_merchant, updated_at: Time.parse('2021-04-01')) # should be counted and in date range
    @transaction_4 = create(:transaction, invoice: @invoice_4)
    @invoice_item_5 = create(:invoice_item, invoice: @invoice_4, item: @item_5, unit_price: 20.00) # $100.00

    @invoice_5 = create(:invoice, customer: @customer, merchant: @other_merchant, updated_at: Time.parse('2021-01-01')) # good invoice, but not in date range
  ...
  context 'both dates are present and valid' do
    it 'returns a revenue object showing the total for that range (inclusive)' do
      ...
      expcet(result[:data]).to be_a Hash
      expect(result[:data]).to have_key :id
      expect(result[:data]).to have_key :attributes

      expect(result[:data][:attributes]).to eq 250.00
    end
  end
